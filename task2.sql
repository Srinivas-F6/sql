...
--2
SELECT name,age,salary, salary+5000 AS new_salary FROM employee;
--3
...
--7
SELECT id, name, age, gender, department, salary, date_of_joining, email FROM employee 
WHERE department = 'SW' AND date_of_joining > TO_DATE('01-01-2020','DD-MM-YYYY');
--8
SELECT id,name,department,email FROM employee WHERE email IS  NULL AND department IS NULL;
--9
SELECT id, name, age, gender, department, salary, date_of_joining, email FROM employee 
WHERE gender = 'Male' AND department = 'Operations' AND salary <= 40000 AND date_of_joining < TO_DATE('15-07-2021','DD-MM-YYYY');
--10
...
SELECT DISTINCT gender, salary, department FROM employee;

SELECT AGE FROM EMPLOYEE;
------IN / NOT IN ------

SELECT id,name,email FROM employee WHERE department IN ('Operations','SWE','HR') AND age  BETWEEN 20 AND 60;


--ORDER BY
SELECT id ,name, age ,gender,salary FROM employee ORDER BY salary ASC,name;

SELECT ID,NAME,EMAIL,department,salary FROM employee ORDER BY salary,department,name;
...
SELECT SUM(SALARY), SUM(AGE) FROM EMPLOYEE;
SELECT MAX(salary) FROM employee WHERE department='HR';
SELECT SUM(SALARY) AS sum_of_salary, AVG(AGE) AS avg_of_ages FROM EMPLOYEE;  


SELECT MIN(SALARY) AS min_salary, MIN(AGE) AS  min_age ,MIN(date_of_joining) AS min_doj FROM EMPLOYEE;


SELECT department,SUM(salary) FROM employee GROUP BY department;

SELECT department,COUNT(gender)AS Male FROM employee WHERE gender = 'Male' GROUP BY department ORDER BY COUNT(GENDER) ASC;

--RANK()  

SELECT department,id,name,gender,salary, RANK() OVER(ORDER BY SALARY DESC) AS rank FROM employee WHERE SALARY IS NOT NULL;

SELECT department,id,name,gender,salary, RANK() OVER(ORDER BY SALARY DESC,age) AS rank FROM employee WHERE SALARY IS NOT NULL;

--ranking according to column then use partition 

SELECT department,id,name,gender,salary, RANK() OVER(PARTITION BY DEPARTMENT ORDER BY SALARY DESC) AS rank FROM employee WHERE SALARY IS NOT NULL;
...
WITH cte AS(
    SELECT id,name,age,salary,department, ROW_NUMBER() OVER(PARTITION BY DEPARTMENT ORDER BY SALARY DESC) AS rn FROM EMPLOYEE WHERE SALARY IS NOT NULL;
)
SELECT * FROM  cte WHERE rn = 7;

WITH cte AS
(
SELECT e.department AS U,e.AVG(SALARY) AS T FROM EMPLOYEE GROUP BY DEPARTMENT;
)
SELECT e.NAME,e.DEPARTMENT,e.SALARY FROM cte C JOIN EMPLOYEE e ON  U = e.DEPARTMENT WHERE SALARY > T;

WITH cte AS (
SELECT id, name, DEPARTMENT ,salary, MAX(SALARY) OVER(PARTITION BY DEPARTMENT) AS maxsal FROM EMPLOYEE WHERE SALARY IS NOT NULL;
)
SELECT * FROM cte WHERE salary = maxsal;
